...
describe("SaleTiers", function() {
  ...
  it("deposit", async function() {
    await expectError("not active", async () => {
      await this.sale.deposit(bn("90"), this.proofs[0], {value: bn("50")});
    });

    await advanceToTime(this.start + 1001);
    await this.sale.deposit(bn("90"), this.proofs[0], {value: bn("50")});

    const userInfo = await this.sale.getUserInfo(this.signer.address);
    ...
    await expectError("need amount > 0", async () => {
      await this.sale.deposit(bn("90"), this.proofs[0], {value: bn("0")});
    });
    await expectError("invalid proof", async () => {
      await this.sale.deposit(bn("90"), this.proofs[1], {value: bn("1")});
    });
    await expectError("invalid proof", async () => {
      await this.sale.connect(this.signers[1]).deposit(bn("10"), this.proofs[0], {value: bn("1")});
    });
    await this.sale.connect(this.signers[1]).deposit(bn("10"), this.proofs[1], {value: bn("10")});

    await this.sale.setPaused(true);
    await expectError("paused", async () => {
      await this.sale.deposit(bn("90"), this.proofs[0], {value: bn("1")});
    });
  });
  ...
  it("harvest", async function() {
    ...
    expect(balanceBefore.sub(balanceAfter).gt(bn('90'))).to.equal(true);
    ...
    expect(balanceAfter.sub(balanceBefore)).to.equal(bn("112.5"));
    ...
    expect(userInfo[3]).to.equal(bn("112.5"));
    expect(userInfo[3].sub(userInfo[1])).to.equal(bn("0"));
    await advanceToTime(this.start + 5001 + (30 * 86400));
    ...
    expect(userInfo[3]).to.equal(bn("225"));
    await advanceToTime(this.start + 5001 + (60 * 86400));
    await this.signer.sendTransaction({ to: this.signer.address, value: bn('1') });
    userInfo = await this.sale.getUserInfo(this.signer.address);
    expect(userInfo[3]).to.equal(bn("337.5"));

    await advanceToTime(this.start + 5001 + (90 * 86400));
    balanceBefore = await this.offeringToken.balanceOf(this.signer.address);
    await this.sale.harvest();
    balanceAfter = await this.offeringToken.balanceOf(this.signer.address);
    expect(balanceAfter.sub(balanceBefore)).to.equal(bn("337.5"));
    userInfo = await this.sale.getUserInfo(this.signer.address);
